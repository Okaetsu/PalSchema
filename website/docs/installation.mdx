...
The next step of this process is making sure you have UE4SS installed properly, and more specifically the right version of it. As of Palworld patch 0.4.1.5, you'll need the Palworld specific version of UE4SS from **[here](https://github.com/Okaetsu/RE-UE4SS/releases/latest)** since they changed some engine related stuff. If you skip this step, you will experience crashing when you start Palworld, you've been warned.

PalSchema must match the appropriate UE4SS version specified in the release notes for whichever PalSchema version you're using. If you get the wrong version, PalSchema will simply not work due to potential API incompatibility.
...
## UE4SS Mod Settings
Before moving forward, you'll want to make sure that your settings for your Mods folder are correct.
...
In mods.txt here, a value of 0 equates to a value of "false" in mods.json, while a value of 1 equates to a value of true. BPModLoaderMod, Keybinds, and BPML_GenericFunctions should all be set to 1 or "true" across both files.

mods.json, and mods.txt basically control if a mod should be enabled or disabled.

If you are using a mod manager like Vortex, then you might see extra entries that you don't see in the picture, and that's perfectly fine. The only other thing you care about is making sure that CheatManagerEnablerMod, ConsoleCommandsMod and ConsoleEnablerMod are disabled because they can cause crashes, although these should be disabled by default.
